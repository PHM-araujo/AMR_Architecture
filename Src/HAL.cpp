...
void HAL::setPinMode(char& port, int& pin, int& mode) {
  auto base = getPtrByPortAndPin(port);
  if (base != nullptr) {
...
void HAL::digitalWriteOnPin(char& port, int& pin, int value) {
  auto base = getPtrByPortAndPin(port);
  if (base != nullptr) {
    if (value) {
...
bool HAL::digitalReadOnPin(char& port, int& pin) {
  auto base = getPtrByPortAndPin(port);
  if (base != nullptr) {
...
volatile uint32_t* HAL::getPtrByPortAndPin(char port) {
